...
//                  assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="" className='w-7 rounded-full' />
...
const ChatContainer = () => {
  ...
  const handleSendImage = async (e)=>{
    ...
    const reader = new FileReader();
    reader.onloadend = async ()=>{
      console.log("2. FileReader loaded, calling sendMessage");
      await sendMessage({image: reader.result})
        console.log("3. sendMessage finished");
      e.target.value = ""
    }
    reader.readAsDataURL(file)
  }
